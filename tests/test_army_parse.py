...
# core/faction abilities are emitted as name-only ability dicts (disabled,
# no-op effect), the same shape as unit abilities
assert [a["name"] for a in u["core_abilities"]] == ['Scouts 7"', "Stealth"]
assert all(a["enabled"] is False and a["effect"]["type"] == "special"
           for a in u["core_abilities"])
